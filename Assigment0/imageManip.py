...
def mix_images(image1, image2, channel1, channel2):
    ...
    mid = int(image1.shape[1]/2)

    out[:,:mid,:] = rgb_exclusion(image1[:,:mid,:],"R")
    out[:,mid:,:] = rgb_exclusion(image2[:,mid:,:],"G")
    ### END YOUR CODE
...
def mix_quadrants(image):
    ...
    ### YOUR CODE HERE
    mid1 = int(image.shape[0]/2)
    mid2 = int(image.shape[1]/2)

    out = np.zeros((image.shape))
    out[:mid1,:mid2,:] = rgb_exclusion(image[:mid1,:mid2,:],"R")
    out[:mid1,mid2:,:] = dim_image(image[:mid1,mid2:,:])
    out[mid1:,:mid2,:] = image[mid1:,:mid2,:] * 0.5
    out[mid1:,mid2:,:] = rgb_exclusion(image[mid1:,mid2:,:],"R")
    ### END YOUR CODE
