...
def energy_function(image):
    ...
    grads = np.gradient(gray_image)
    out = np.abs(grads[0]) + np.abs(grads[1])
    ### END YOUR CODE
...
def remove_seam(image, seam):
    ...
    ### YOUR CODE HERE
    for row in range(H):
    	out[row,seam[row]:W-1,:] = out[row, seam[row]+1:,:]
    out = out[:,:W-1,:]
    ### END YOUR CODE
...
def duplicate_seam(image, seam):
    ...
    ### YOUR CODE HERE
    for row in range(H):
    	#out[row,:seam[row],:] = image[row,:seam[row],:]
    	#out[row,seam[row],:] = image[row,seam[row],:]
    	#if seam[row] < W-1:
    	#	out[row,seam[row]+1:,:] = np.concatenate((image[row,seam[row]+1,:].reshape(1,1,C),image[row,seam[row]+1:,:].reshape(1,-1,C)),axis=1)
    	out[row] = np.insert(image[row],seam[row],image[row,seam[row]],axis=0)
    ### END YOUR CODE
