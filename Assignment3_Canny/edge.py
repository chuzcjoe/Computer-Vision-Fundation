...
def gradient(img):
    ...
    G = np.sqrt(Gx**2+Gy**2)
    			#theta[i,j] = np.arctan2((Gx[i,j]/Gy[i,j]))
...
def link_edges(strong_edges, weak_edges):
    ...
    for i in range(indices.shape[0]):
    	neighbors = []
    	neighbors = get_neighbors(indices[i][0],indices[i][1],H,W)
...
def hough_transform(img):
    ...
    for i,j in zip(ys,xs):
    	for thet in range(thetas.shape[0]):
    		 r = j * cos_t[thet] + i * sin_t[thet]
    		 accumulator[int(r+diag_len),thet] += 1
    ### END YOUR CODE
